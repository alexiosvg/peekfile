#!/bin/bash
##Troubleshoot the case of which not both arguments are provided.
...
if [[ $# -eq 1 ]]; then
  ...
  N="$2"
    ...
    echo "Usage: ./fastascan.ai.sh <folder> [N]"
    exit 1
fi
...
##WHAT IF A FILE IS EMPTY
if [[ $file_count -eq 0 ]]; then
  echo "No FASTA files found in the folder: $folder"
  exit 0
fi
